...
var isCrappyIE = (navigator.userAgent.match(/msie 6/i));
...
soundManager.onload = function() {
  ...
  soundManager.createSound({id:'mouseOver',url:'demo/_mp3/mouseover3.mp3',autoLoad:!isCrappyIE});
...
function doResize() {
  ...
  screenY = (window.innerHeight?window.innerHeight:document.documentElement.clientHeight||document.body.clientHeight);
}

doResize();

function parallax(e) {
  var x = parseInt(-16*(e.clientX/screenX));
  var y = parseInt(-32*(e.clientY/screenY));
...
function undoChicken() {
  var rnd = 2+parseInt(Math.random()*5);
  for (var i=0; i<rnd; i++) {
...
function preloadChicken() {
  ...
  if (!soundManager.sounds['background0']) {
    for (i=0; i<3; i++) {
      soundManager.createSound({
	    id:'background'+i,
	    url:'demo/_mp3/background'+i+'.mp3',
	    autoLoad:true
	  });
    }
  }
...
function timeCheck() {
  ...
  if (hour<8 || hour>17 && !window.location.toString().match('day')) toggleTheme(); // night time
}
...
function setRainbow() {
  if (isCrappyIE) return false;
  $('header').className = 'rainbow';
  $('version').innerHTML = 'ZOMGWTF those aren\'t even in the right <i>order</i>, n00b designer LOLZ';
  if (chicken) {
    window.setTimeout(function(){mak();undoChicken()},1000);
  }
